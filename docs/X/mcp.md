...
This project uses the X API MCP full route, not the app-only bearer route.

The project-level MCP configuration lives in `.mcp.json` and starts the local `xurl`
bridge for X API calls. It also registers the X documentation MCP:
...
`scripts/x-mcp.mjs` reads `.env`, maps `X_CLIENT_ID` / `X_CLIENT_SECRET` to
the `CLIENT_ID` / `CLIENT_SECRET` names expected by `xurl`, and then launches:
...
The full route uses OAuth 2.0 user context through `@xdevplatform/xurl`. The X
developer app must have OAuth 2.0 enabled and the redirect URI
`http://localhost:8080/callback` registered. On first use, `xurl` opens a
browser login and then reuses the cached token.

The app's Codex Reset Notifier uses OAuth 1.0a separately. It accepts either
`X_API_KEY` / `X_API_SECRET` or the equivalent `X_CONSUMER_KEY` /
`X_CONSUMER_SECRET`, plus:
