...
description: 从零完成示例项目的本地部署与初始配置 —— 检测环境、安装运行时、克隆仓库、安装依赖、启动开发服务器
---

# 项目本地部署 skill

本 skill 指导 AI agent 从 0 完成一个已具备完整功能的 Web 示例项目的本地部署与初始配置，全程无需用户介入执行命令。

示例项目的基础能力（典型形态，按实际项目描述）：

- 批量图片抓取 / 搬运
- 营销物料（轮播图、详情页、社媒封面等）的批量生成
...
仓库地址由用户在使用时提供。执行 `git clone <repo-url>`，进入项目目录。
...
在终端中执行 `pnpm dev`，等本地地址出现（通常是 `http://localhost:3000`）。
...
部署完成。用户可以开始基于本项目开发自有业务功能。下一步可以：

- 参考 `project-model-integration` skill 接入新的图片 / 视频模型
