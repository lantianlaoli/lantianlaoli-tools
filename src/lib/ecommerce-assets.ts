...
export const CHUB_TWO_PERSON_URL =
  "https://i.postimg.cc/gknHRkbY/Screen-Shot-2026-06-16-152220-854.png";
export const CHUB_TWO_MAIN_COMPOSITION_URL =
  "https://i.postimg.cc/GtqBSY09/7604c462-8537-4270-ae35-0646cce18859.png";
export const CHUB_TWO_DEFAULT_STYLE_GUIDE =
...
export const CHUB_TWO_DEFAULT_GENERATION_CONFIG: EcommerceGenerationConfig = {
  ...
  person: {
    ...
    prompt:
      "Use this person only in the main carousel image. Place the person in the lower-right half, with the right hand reaching forward and holding the primary SKU.",
  },
  logo: {
    imageUrl: CHUB_TWO_LOGO_URL,
    prompt:
      "Use this exact CHUB TWO logo in the top-left corner of carousel images except the first main image. Do not redraw, duplicate, or place it elsewhere.",
  },
  mainComposition: {
    imageUrl: CHUB_TWO_MAIN_COMPOSITION_URL,
    prompt:
      "Use this image for layout only: headline upper-left, person and product lower-right. Do not copy its product, text, or brand marks.",
  },
};
export const ECOMMERCE_CAROUSEL_ROLE_COUNTS: Record<
  EcommerceCarouselRole,
  number
> = { main: 1, scene: 4, detail: 3, variant: 1 };
export const ECOMMERCE_REFERENCE_ROLE_MAX_COUNTS: Partial<
  Record<EcommerceCarouselRole, number>
> = { scene: 4, detail: 3, variant: 1 };
export const ECOMMERCE_CAROUSEL_ROLES: EcommerceCarouselRole[] = [
  "main",
  "scene",
  "detail",
  "variant",
...
const ROLE_GUIDANCE: Record<EcommerceCarouselRole, string> = {
  ...
  scene:
    "Show one distinct believable futuristic lifestyle use scene for young consumers. Keep the product dominant and make the use case immediately understandable.",
  detail:
    "Use a tight, filled composition to show one distinct material, structure, function, or benefit. Do not invent features that are not visible in the references.",
  variant:
    "Clearly compare the actual SKU differences in color, form, bundle, or specification using a clean, minimal comparison composition and no unsupported claims.",
};

export function getEcommerceCarouselSlots(
        ...
        length:
          role === "main"
            ...
            : role === "variant"
              ? 1
              : Math.min(
                  Math.max(referenceCounts[role] ?? 0, 0),
...
function roleFallback(
  ...
  const content: Record<
    EcommerceCarouselRole,
    Array<[string, string]> | Record<number, Array<[string, string]>>
  > = {
    main: [
      ["Meet the next move", "Designed for your everyday edge."],
      ["Control your flow", "A smarter form for a faster life."],
      ["Built for what is next", "Focused design. Instant impact."],
    ],
    scene: {
      1: [
        ["Make space for focus", "A clean setup for the moments that matter."],
        ["Power your next move", "Designed to keep your day in motion."],
        ["Your desk, reimagined", "Less clutter. More control."],
      ],
      2: [
        ["Built into the rhythm", "Smart utility for everyday momentum."],
        ["One move ahead", "A focused tool for a faster routine."],
        ["Keep your flow", "Simple power for the way you work."],
      ],
      3: [
        ["Ready when you are", "A refined companion for modern spaces."],
        ["Less noise, more signal", "Technology that stays out of the way."],
        ["Turn on the future", "Designed for the next generation of desks."],
      ],
    },
    detail: {
      1: [
        ["Every detail has a reason", "Refined materials. Clear function."],
        ["Precision, made visible", "See the construction behind the form."],
        ["Form follows focus", "Nothing extra. Everything intentional."],
      ],
      2: [
        [
          "Small form. Serious intent.",
          "Designed around the details that matter.",
        ],
        ["The power is in the build", "A closer look at what makes it work."],
        [
          "Clean outside. Smart inside.",
          "Engineering shaped for everyday use.",
        ],
      ],
      3: [
        ["See the difference", "Precision you can feel and trust."],
        ["Designed to stay clear", "Every surface and edge has a purpose."],
        [
          "The detail that changes everything",
          "A closer look at real product value.",
        ],
      ],
    },
    variant: {
      1: [
        ["Choose your signal", "Compare the details. Find your fit."],
        ["Your setup, your way", "A clear look at every real option."],
        ["One form, different modes", "Find the configuration that fits."],
      ],
      2: [
        ["More than one way forward", "Explore the available configurations."],
        [
          "Built to match your setup",
          "Compare the real differences at a glance.",
        ],
        ["Pick your next move", "The right variant for your daily rhythm."],
      ],
    },
  };
  const entries = Array.isArray(content[role])
    ? (content[role] as Array<[string, string]>)
    : (content[role][index] ?? content[role][1]);
  return entries.map(([title, subtitle], proposalIndex) => ({
    id: `${prefix}-${proposalIndex + 1}`,
    title,
    subtitle,
  }));
}

export function fallbackEcommerceCopyOptions(
  referenceCounts: Partial<Record<EcommerceCarouselRole, number>> = {},
): EcommerceSlotCopyOptions[] {
  return getEcommerceCarouselSlots(referenceCounts).map((slot) => ({
    ...slot,
    slotId: slot.id,
    proposals: roleFallback(slot.role, slot.index),
  }));
}
...
export async function analyzeEcommerceCopy(input: {
  ...
  const referenceUrls = ECOMMERCE_CAROUSEL_ROLES.flatMap(
    (role) => input.manufacturerReferenceImageUrls[role] ?? [],
  );
  const message: OpenRouterMessage = {
    ...
    content: [
      {
        type: "text",
        text: buildEcommerceCopyAnalysisPrompt({
  ...
  const finalize = (slots: EcommerceSlotCopyOptions[]) => {
    const finalSlots = slots.map((slot) => ({
      ...slot,
      proposals: [slot.proposals[0]],
    }));
    return {
      slots: finalSlots,
      copyBySlot: Object.fromEntries(
        finalSlots.map((slot) => [slot.slotId, slot.proposals[0]]),
      ) as Record<string, EcommerceCopyProposal>,
    };
  };
  try {
    const result = await callOpenRouter<{ slots?: EcommerceSlotCopyOptions[] }>(
      [message],
      { type: "json_object" },
    );
    return {
      ...finalize(normalizeCopyOptions(result.slots, referenceCounts)),
      usedFallback: false,
    };
  } catch (error) {
    console.warn(
      "[ecommerce-assets/analyze] Falling back to deterministic copy options:",
      error instanceof Error ? error.message : error,
    );
    return {
      ...finalize(fallbackEcommerceCopyOptions(referenceCounts)),
      usedFallback: true,
    };
  }
}
...
export function normalizeCopyOptions(
  ...
  return getEcommerceCarouselSlots(referenceCounts).map((slot) => {
    const candidate = candidates.find(
      (item) => (item as { slotId?: unknown })?.slotId === slot.id,
    ) as Partial<EcommerceSlotCopyOptions> | undefined;
    const proposals = Array.isArray(candidate?.proposals)
      ? candidate.proposals
          .filter((proposal): proposal is EcommerceCopyProposal =>
            Boolean(
              proposal &&
                typeof proposal.title === "string" &&
                typeof proposal.subtitle === "string" &&
                proposal.title.trim() &&
          ...
          .map((proposal, index) => ({
            ...
            subtitle: proposal.subtitle.trim(),
          }))
      : [];
    const signature = proposals
      .map((proposal) => `${proposal.title}|${proposal.subtitle}`)
      .join("\n");
    const seen = seenByRole.get(slot.role) ?? new Set<string>();
    const normalizedProposals =
      proposals.length === 3 && !seen.has(signature)
        ? proposals
        : roleFallback(slot.role, slot.index);
    seen.add(
      normalizedProposals
        .map((proposal) => `${proposal.title}|${proposal.subtitle}`)
        .join("\n"),
...
export function buildEcommerceCarouselPrompts(input: {
  ...
  manufacturerReferenceCountByRole: Partial<
    Record<EcommerceCarouselRole, number>
  >;
  styleGuide?: string;
  ...
  Pick<
    EcommerceImageSlot,
    "id" | "role" | "index" | "title" | "prompt" | "usePerson" | "selectedCopy"
  >
> {
  const styleGuide = input.styleGuide?.trim() || CHUB_TWO_DEFAULT_STYLE_GUIDE;
  const config = input.generationConfig ?? CHUB_TWO_DEFAULT_GENERATION_CONFIG;
  return getEcommerceCarouselSlots(input.manufacturerReferenceCountByRole).map(
    (slot) => {
      ...
      const prompt = [
        ...
        slot.role === "main"
          ? `There are three input images in this order: image 1 is the ONLY product identity source (SKU ${input.primarySkuIndex + 1}); image 2 is the ONLY layout reference (${config.mainComposition.imageUrl}); image 3 is the ONLY person reference. Create the hero image using the product from image 1, the layout from image 2, and the person from image 3. ${config.mainComposition.prompt} Replace the product and copy from the reference image completely. Do not copy the reference product, text, logo, or brand marks. Never show any other SKU.`
          : slot.role === "variant"
        ...
        slot.role === "variant"
          ? "Create this variant/specification image entirely from the uploaded SKU product images. Do not use or expect a manufacturer variant reference image."
          : "",
        slot.role === "scene" || slot.role === "detail"
          ? `This is ${slot.role} slot ${slot.index}; make its visual topic and composition distinct from the other ${slot.role} slots.`
          : "",
        ...
        usePerson
          ? `Use the configured person reference from ${config.person.imageUrl}. ${config.person.prompt}`
          : "Do not use the configured person reference in this image.",
        slot.role === "main"
          ? "Do not add the configured logo to the first white-background main image."
          : `MANDATORY BRANDING RULE: The configured CHUB TWO logo from ${config.logo.imageUrl} must appear exactly once in the top-left corner with a clear margin. Treat the logo reference as input image 1. Do not omit it, redraw it, move it, duplicate it, or place it in any other corner. ${config.logo.prompt}`,
