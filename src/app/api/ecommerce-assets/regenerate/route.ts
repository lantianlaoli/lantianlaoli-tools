...
export async function POST(request: Request) {
  try {
    ...
    if (!job || !slotId || !title || !subtitle)
      return NextResponse.json(
        { error: "job, slotId, title, and subtitle are required." },
        { status: 400 },
      );
    const slot = job.carouselImages.find(
      (candidate) => candidate.id === slotId,
    );
    if (!slot)
      return NextResponse.json(
        { error: "Image slot was not found." },
        { status: 404 },
      );
    const skuIndex = Math.min(
      Math.max(body.skuIndex ?? 0, 0),
      job.productSkuImageUrls.length - 1,
    );
    ...
    const prompts = buildEcommerceCarouselPrompts({
      ...
      selectedCopyBySlot: {
        [slot.id]: { id: `${slot.id}-regenerated`, title, subtitle },
      },
    ...
    if (!promptSlot)
      return NextResponse.json(
        { error: "Image slot configuration was not found." },
        { status: 400 },
      );
    const inputUrls =
      slot.role === "main"
        ? [
            job.productSkuImageUrls[skuIndex],
            ...getSlotReferenceUrls(job, { ...slot, usePerson: true }).filter(
              (url) => !job.productSkuImageUrls.includes(url),
            ),
          ]
        : slot.role === "variant"
          ? [
              ...(job.brandLogo?.enabled ? [job.brandLogo.logoImageUrl] : []),
              ...job.productSkuImageUrls,
            ]
          : [
              ...(job.brandLogo?.enabled ? [job.brandLogo.logoImageUrl] : []),
    ...
    const taskId = await createKieImageTask({
      prompt: [promptSlot.prompt, refinementInstruction]
        .filter(Boolean)
        .join("\n\n"),
      inputUrls: [
        ...(slot.role === "variant" ? [] : [slot.resultUrl]),
        ...inputUrls,
    ...
    return NextResponse.json({
      ...
      prompt: [promptSlot.prompt, refinementInstruction]
        .filter(Boolean)
        .join("\n\n"),
      selectedCopy: { id: `${slot.id}-regenerated`, title, subtitle },
      skuIndex,
    });
  } catch (error) {
