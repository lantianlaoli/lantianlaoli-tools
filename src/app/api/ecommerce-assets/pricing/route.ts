...
import {
  buildTikTokPricingPrompt,
  calculateTikTokPricing,
  tiktokPricingRequestSchema,
...
export async function POST(request: Request) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      { error: "Invalid pricing inputs.", details: parsed.error.flatten() },
      { status: 400 },
    );
  ...
  if (process.env.OPENROUTER_API_KEY && process.env.OPENROUTER_MODEL) {
    try {
      ai = await callOpenRouter<TikTokPricingAiRecommendation>(
        [
          { role: "system", content: "你只输出合法 JSON，不要 Markdown。" },
          {
            role: "user",
            content: buildTikTokPricingPrompt(parsed.data, calculation),
          },
        ],
        { type: "json_object" },
      );
    } catch (error) {
      aiError = error instanceof Error ? error.message : "AI recommendation failed.";
    }
  } else {
    aiError = "未配置 OPENROUTER_API_KEY / OPENROUTER_MODEL，已返回公式计算结果。";
  }
